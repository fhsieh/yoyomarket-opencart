...
class Mail {
	...
	public function send() {
		if (!$this->to) {
			trigger_error('Error: To email required!');
			exit();
		}

		if (!$this->from) {
			trigger_error('Error: From email required!');
			exit();
		}

		if (!$this->sender) {
			trigger_error('Error: Sender email required!');
			exit();
		}
...
//			$header .= 'Reply-To: =?UTF-8?B?' . base64_encode($this->reply_to) . '?=' . ' <' . $this->reply_to . '>' . $this->newline;
			$header .= 'Reply-To: <' . $this->reply_to . '>' . $this->newline; // yym_custom
		}
