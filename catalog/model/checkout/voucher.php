...
// yym_custom				$data['title'] = sprintf($language->get('text_subject'), $voucher['from_name']);
                ...
                $data['code'] = $voucher['code']; // yym_custom
				...
				$data['logo'] = $this->config->get('config_url') . 'image/' . $this->config->get('config_logo'); // yym_custom
...
// yym_custom				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/mail/voucher.tpl')) {
// yym_custom					$html = $this->load->view($this->config->get('config_template') . '/template/mail/voucher.tpl', $data);
// yym_custom				} else {
// yym_custom					$html = $this->load->view('default/template/mail/voucher.tpl', $data);
// yym_custom				}
                ...
                if ($voucher['message']) {
                  ...
                  $text .= $voucher['message'] . "\n\n";
                }
                $text .= $language->get('text_code') . ': ' . $voucher['code'] . "\n\n";
