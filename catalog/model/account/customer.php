...
//		$text .= html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8');
		...
		if ($this->config->get('config_account_mail')) {
			...
			$text .= $this->language->get('text_website') . ' ' . html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8') . "\n";
			$text .= $this->language->get('text_firstname') . ' ' . $data['firstname'] . "\n";
			$text .= $this->language->get('text_lastname') . ' ' . $data['lastname'] . "\n";
			$text .= $this->language->get('text_customer_group') . ' ' . $customer_group_info['name'] . "\n";
			$text .= $this->language->get('text_email') . ' '  .  $data['email'] . "\n";
			$text .= $this->language->get('text_telephone') . ' ' . $data['telephone'] . "\n";
    ...
    // yym_custom: temporary password to redirect customer on login
    public function editPasswordStatus($email, $status) {
        $this->db->query("UPDATE " . DB_PREFIX . "customer SET password_status = '" . (int)$status . "' WHERE LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) . "'");
    }

    public function getPasswordStatus() {
        $customer_id = $this->customer->getId();

        $query = $this->db->query("SELECT password_status FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int)$customer_id . "'");

        return $query->row['password_status'];
    }
