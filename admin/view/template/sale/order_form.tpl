		...
		success: function(json) {
			...
			if (json['products']) {
				for (i = 0; i < json['products'].length; i++) {
					...
					html += '<tr>';
					html += '  <td class="text-left">' + product['name'] + ' ' + (!product['stock'] ? '<span class="text-danger"><i class="fa fa-exclamation-circle"></i></span>' : '') + '<br />'; // yym_custom: style for quantity not available
